...
class QQChannel(BaseChannel):
    ...
    async def start(self) -> None:
        if not QQ_AVAILABLE:
            logger.error("[{}] qq-botpy not installed, run: pip install qq-botpy", self.name)
            return

        if not self._app_id or not self._secret:
            logger.error("[{}] app_id or secret not configured", self.name)
            return
        ...
        logger.info("[{}] QQ bot started (C2C direct message mode)", self.name)
    ...
    async def _run_bot(self) -> None:
        while self._running:
            try:
                ...
                logger.warning("[{}] QQ bot error: {}", self.name, e)
            if self._running:
                logger.info("[{}] Reconnecting QQ bot in 5 seconds...", self.name)
    ...
    async def stop(self) -> None:
        ...
        if self._bot_task:
            ...
            try:
                ...
                pass
        logger.info("[{}] QQ bot stopped", self.name)

    # ── Outbound: send message ─────────────────────────────────────────────────

    async def send(self, chat_id: str, content: str) -> None:
        if not self._client:
            logger.warning("[{}] QQ client not initialized", self.name)
            return
        ...
        try:
            ...
            logger.error("[{}] Failed to send QQ message: {}", self.name, e)
    ...
    async def _on_message(self, data: "C2CMessage") -> None:
        try:
            ...
            logger.error("[{}] Error processing QQ message: {}", self.name, e)
