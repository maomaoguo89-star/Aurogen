...
class MochatChannel(BaseChannel):
    ...
    settings:
        claw_token                  : Mochat Claw API token
        base_url                    : Mochat server base URL
        sessions                    : List of session IDs to watch (["*"] for auto-discovery)
        panels                      : List of panel IDs to watch (["*"] for auto-discovery)
        agent_user_id               : Bot's own user ID (filter own messages, detect mentions)
        socket_url                  : Socket.IO connection URL (default: base_url)
        socket_path                 : Socket.IO path (default: "/socket.io")
        socket_disable_msgpack      : Disable msgpack serialization (default: false)
        socket_reconnect_delay_ms   : Initial reconnect delay ms (default: 1000)
        socket_max_reconnect_delay_ms: Max reconnect delay ms (default: 5000)
        socket_connect_timeout_ms   : Connection timeout ms (default: 10000)
        watch_timeout_ms            : HTTP watch timeout ms (default: 30000)
        watch_limit                 : Messages per fetch (default: 50)
        max_retry_attempts          : Max retry attempts, 0=unlimited (default: 0)
        retry_delay_ms              : Retry delay ms (default: 3000)
        refresh_interval_ms         : Target refresh interval ms (default: 60000)
        mention_require_in_groups   : Require mention in groups to reply (default: false)
        groups                      : Group config dict {group_id: {require_mention: bool}}
        reply_delay_mode            : Reply delay mode ("non-mention" or "") (default: "")
        reply_delay_ms              : Reply delay ms (default: 3000)
    """

    def __init__(self, channel_key: str, settings: dict):
        self.name = channel_key

        self._claw_token: str = settings.get("claw_token", "")
        self._base_url: str = settings.get("base_url", "")
        self._sessions_cfg: list = settings.get("sessions", [])
        self._panels_cfg: list = settings.get("panels", [])
        self._agent_user_id: str = settings.get("agent_user_id", "")

        self._socket_url: str = settings.get("socket_url", "")
        self._socket_path: str = settings.get("socket_path", "/socket.io")
        self._socket_disable_msgpack: bool = bool(settings.get("socket_disable_msgpack", False))
        self._socket_reconnect_delay_ms: int = int(settings.get("socket_reconnect_delay_ms", 1000))
        self._socket_max_reconnect_delay_ms: int = int(settings.get("socket_max_reconnect_delay_ms", 5000))
        self._socket_connect_timeout_ms: int = int(settings.get("socket_connect_timeout_ms", 10000))

        self._watch_timeout_ms: int = int(settings.get("watch_timeout_ms", 30000))
        self._watch_limit: int = int(settings.get("watch_limit", 50))
        self._max_retry_attempts: int = int(settings.get("max_retry_attempts", 0))
        self._retry_delay_ms: int = int(settings.get("retry_delay_ms", 3000))
        self._refresh_interval_ms: int = int(settings.get("refresh_interval_ms", 60000))

        self._mention_require_in_groups: bool = bool(settings.get("mention_require_in_groups", False))
        self._groups: dict = settings.get("groups", {})
        self._reply_delay_mode: str = settings.get("reply_delay_mode", "")
        self._reply_delay_ms: int = int(settings.get("reply_delay_ms", 3000))
        ...
        self._state_dir = Path.home() / ".aurogen" / "mochat"
    ...
    async def start(self) -> None:
        if not self._claw_token:
            logger.error("[{}] claw_token not configured", self.name)
            return
        ...
        logger.info("[{}] Mochat channel started", self.name)
    ...
    async def stop(self) -> None:
        ...
        if self._http:
            ...
            self._http = None
        self._ws_connected = self._ws_ready = False
        logger.info("[{}] Mochat channel stopped", self.name)

    async def send(self, chat_id: str, content: str) -> None:
        if not self._claw_token:
            logger.warning("[{}] claw_token missing, skip send", self.name)
            return
        ...
        if not target.id:
            logger.warning("[{}] outbound target is empty", self.name)
            return

        is_panel = (target.is_panel or target.id in self._panel_set) and not target.id.startswith("session_")
        try:
            if is_panel:
                await self._api_send("/api/claw/groups/panels/send", "panelId", target.id, content.strip())
            else:
                await self._api_send("/api/claw/sessions/send", "sessionId", target.id, content.strip())
        except Exception as e:
            logger.error("[{}] failed to send Mochat message: {}", self.name, e)

    # ---- config / init helpers ---------------------------------------------

    def _seed_targets_from_config(self) -> None:
        sessions, self._auto_discover_sessions = self._normalize_id_list(self._sessions_cfg)
        panels, self._auto_discover_panels = self._normalize_id_list(self._panels_cfg)
    ...
    @staticmethod
    def _normalize_id_list(values: list) -> tuple[list[str], bool]:
        cleaned = [str(v).strip() for v in values if str(v).strip()]
        return sorted({v for v in cleaned if v != "*"}), "*" in cleaned

    # ---- mention resolution (instance method) ------------------------------

    def _resolve_require_mention(self, session_id: str, group_id: str) -> bool:
        groups = self._groups or {}
        for key in (group_id, session_id, "*"):
            if key and key in groups:
                g = groups[key]
                if isinstance(g, dict):
                    return bool(g.get("require_mention", False))
                return bool(g)
        return bool(self._mention_require_in_groups)

    # ---- websocket ---------------------------------------------------------

    async def _start_socket_client(self) -> bool:
        if not SOCKETIO_AVAILABLE:
            logger.warning("[{}] python-socketio not installed, using HTTP polling fallback", self.name)
            return False

        serializer = "default"
        if not self._socket_disable_msgpack:
            if MSGPACK_AVAILABLE:
                serializer = "msgpack"
            else:
                logger.warning("[{}] msgpack not installed but socket_disable_msgpack=false, using JSON", self.name)

        client = socketio.AsyncClient(
            reconnection=True,
            reconnection_attempts=self._max_retry_attempts or None,
            reconnection_delay=max(0.1, self._socket_reconnect_delay_ms / 1000.0),
            reconnection_delay_max=max(0.1, self._socket_max_reconnect_delay_ms / 1000.0),
        ...
        async def connect() -> None:
            ...
            logger.info("[{}] Mochat websocket connected", self.name)
        ...
        async def disconnect() -> None:
            if not self._running:
                return
            self._ws_connected = self._ws_ready = False
            logger.warning("[{}] Mochat websocket disconnected", self.name)
            await self._ensure_fallback_workers()

        @client.event
        async def connect_error(data: Any) -> None:
            logger.error("[{}] Mochat websocket connection error: {}", self.name, data)
        ...
        socket_url = (self._socket_url or self._base_url).strip().rstrip("/")
        socket_path = (self._socket_path or "/socket.io").strip().lstrip("/")

        try:
            self._socket = client
            await client.connect(
                socket_url, transports=["websocket"], socketio_path=socket_path,
                auth={"token": self._claw_token},
                wait_timeout=max(1.0, self._socket_connect_timeout_ms / 1000.0),
            )
            return True
        except Exception as e:
            logger.error("[{}] Mochat websocket connection failed: {}", self.name, e)
            try:
    ...
    async def _subscribe_sessions(self, session_ids: list[str]) -> bool:
        ...
        if not ack.get("result"):
            logger.error("[{}] subscribeSessions failed: {}", self.name, ack.get("message", "unknown error"))
            return False
    ...
    async def _subscribe_panels(self, panel_ids: list[str]) -> bool:
        ...
        if not ack.get("result"):
            logger.error("[{}] subscribePanels failed: {}", self.name, ack.get("message", "unknown error"))
            return False
        return True
    ...
    async def _refresh_loop(self) -> None:
        interval_s = max(1.0, self._refresh_interval_ms / 1000.0)
        while self._running:
            await asyncio.sleep(interval_s)
            try:
                await self._refresh_targets(subscribe_new=self._ws_ready)
            except Exception as e:
                logger.warning("[{}] Mochat refresh failed: {}", self.name, e)
    ...
    async def _refresh_sessions_directory(self, subscribe_new: bool) -> None:
        try:
            ...
            logger.warning("[{}] listSessions failed: {}", self.name, e)
            return
    ...
    async def _refresh_panels(self, subscribe_new: bool) -> None:
        try:
            ...
            logger.warning("[{}] getWorkspaceGroup failed: {}", self.name, e)
            return
        ...
        while self._running and self._fallback_mode:
            try:
                payload = await self._post_json("/api/claw/sessions/watch", {
                    ...
                    "timeoutMs": self._watch_timeout_ms, "limit": self._watch_limit,
                })
                await self._handle_watch_payload(payload, "session")
            except asyncio.CancelledError:
                break
            except Exception as e:
                logger.warning("[{}] watch fallback error ({}): {}", self.name, session_id, e)
                await asyncio.sleep(max(0.1, self._retry_delay_ms / 1000.0))

    async def _panel_poll_worker(self, panel_id: str) -> None:
        sleep_s = max(1.0, self._refresh_interval_ms / 1000.0)
        while self._running and self._fallback_mode:
            try:
                resp = await self._post_json("/api/claw/groups/panels/messages", {
                    "panelId": panel_id, "limit": min(100, max(1, self._watch_limit)),
                })
                ...
                logger.warning("[{}] panel poll error ({}): {}", self.name, panel_id, e)
        ...
        async with lock:
            ...
            for event in raw_events:
                if not isinstance(event, dict):
                    continue
                seq = event.get("seq")
                if target_kind == "session" and isinstance(seq, int) and seq > self._session_cursor.get(target_id, 0):
    ...
    async def _process_inbound_event(self, target_id: str, event: dict[str, Any], target_kind: str) -> None:
        ...
        if not author or (self._agent_user_id and author == self._agent_user_id):
            return
        ...
        was_mentioned = resolve_was_mentioned(payload, self._agent_user_id)
        require_mention = target_kind == "panel" and is_group and self._resolve_require_mention(target_id, group_id)
        use_delay = target_kind == "panel" and self._reply_delay_mode == "non-mention"
    ...
    async def _delay_flush_after(self, key: str, target_id: str, target_kind: str) -> None:
        await asyncio.sleep(max(0, self._reply_delay_ms) / 1000.0)
    ...
    async def _dispatch_entries(self, target_id: str, target_kind: str, entries: list[MochatBufferedEntry], was_mentioned: bool) -> None:
        ...
        session_id = f"{self.name}@{target_id}"
    ...
    async def _load_session_cursors(self) -> None:
        ...
        try:
            ...
            logger.warning("[{}] failed to read cursor file: {}", self.name, e)
            return
    ...
    async def _save_session_cursors(self) -> None:
        try:
            ...
            logger.warning("[{}] failed to save cursor file: {}", self.name, e)
    ...
    async def _post_json(self, path: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        url = f"{self._base_url.strip().rstrip('/')}{path}"
        response = await self._http.post(url, headers={
            "Content-Type": "application/json", "X-Claw-Token": self._claw_token,
        }, json=payload)
    ...
    async def _api_send(self, path: str, id_key: str, id_val: str, content: str) -> dict[str, Any]:
